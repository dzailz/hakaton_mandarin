...
class TestDataPreparation:
    def test_is_normality_distributed(self, normal_data_preparation):
        ...
        for column in ["age", "income"]:
            assert column in distribution, f"{column.capitalize()} is a numeric column, but was not included in the output"
            assert distribution[column]["is_normal"], f"{column.capitalize()} is normally distributed, but the output is {distribution[column]['is_normal']}"
    ...
    def test_is_not_normality_distributed(self, lognormal_data_preparation):
        ...
        # Check the output for 'age'
        assert "age" in distribution, "Error: 'age' is a numeric column, but was not included in the output"
        assert not distribution["age"]["is_normal"], f"Error: 'age' is normally distributed, but the output is {distribution['age']['is_normal']}"

        # Check the output for 'income'
        assert "income" in distribution, "Error: 'income' is a numeric column, but was not included in the output"
        assert not distribution["income"]["is_normal"], f"Error: 'income' is normally distributed, but the output is {distribution['income']['is_normal']}"
    ...
    def test_add_time_features(self):
        ...
        data = {"birth_date": pd.date_range(start="1990-01-01", periods=3), "job_start_date": pd.date_range(start="2010-01-01", periods=3)}
